...
const AddMeeting = (props) => {
    ...
    const [isLoading, setIsLoading] = useState(false);  //Fix : Wrong spelling isLoding
    ...
    const AddData = async (formData, resetForm) => {
        try {
            setIsLoading(true);
            const response = await postApi('api/meeting', formData);
            console.log("API Response:", response); // Log response for debugging
    
            if (response.status === 201) {
                toast.success('Meeting added successfully');
                fetchData(); // ERROR: fetchData is not a function
                resetForm();
                onClose();
            } else {
    ...
    const fetchAllData = async () => {
        try {
            setIsLoading(true);
            const meetingRes = await getApi('api/meeting'); // Fetch only meetings
            console.log("Meeting Data:", meetingRes.data); // Log data to console
        } catch (error) {
    ...
    return (
        <Modal onClose={onClose} isOpen={isOpen} isCentered>
            ...
            <ModalContent height={"580px"}>
                ...
                <ModalFooter>
                    <Button size="sm" variant='brand' me={2} disabled={isLoading ? true : false} onClick={handleSubmit}>{isLoading ? <Spinner /> : 'Save'}</Button>
                    <Button sx={{
